...
// origin is the identity). The unsigned variant still uses
...
// rpcert.RegistrationRef.Claims() ([ETSI TS 119 475 §5.2.4]) —
...
func (e *Engine) dcapiClaims(s *Session, signed bool) (map[string]any, error) {
	...
	// flag on (mirrors requestClaims; transaction_data scope
	// note).
...
// origin validation) and the DCAPI handover. Fail closed:
...
func (e *Engine) ProcessDCAPIResponse(ctx context.Context, s *Session, origin string, data []byte) ([]Presentation, error) {
	...
	// [OID4VP §8.2] state binding — wrong/absent origin rejected before decryption.
